...
/*
** Returns round up value of log2(n).
** Note: it is used at runtime.
*/
int tlsf_log2_ceil(size_t n)
{
	int bits = tlsf_fls_sizet(n);
	if (((size_t)1 << bits) < n)
	{
		bits++;
	}
...
typedef struct control_t
{
	...
	size_t sl_index_count : 8;
	size_t fl_index_count : 8;
	size_t fl_index_max : 8;
	size_t fl_index_shift : 8;
	size_t sl_index_count_log2 : 8;
#else
	size_t small_block_size : 8;
	size_t sl_index_count : 6;
	size_t fl_index_count : 5;
	size_t fl_index_max : 6;
	size_t fl_index_shift : 4;
	/* log2 of number of linear subdivisions of block sizes. Larger
	** values require more memory in the control structure. Values of
	** 4 or 5 are typical.*/
	size_t sl_index_count_log2 : 3;
#endif
...
const size_t control_size = sizeof(control_t);
...
static control_t *control_construct(control_t *control, size_t mem_size, size_t max_pool_size)
{
	/* Find the closest power of two for first layer */
	control->fl_index_max = tlsf_log2_ceil(max_pool_size);

	/* Adapt second layer to the pool */
	// if (max_pool_size <= 16 * 1024)
	// 	control->sl_index_count_log2 = 3; // maybe better for low memory mcu
	// else if (max_pool_size <= 256 * 1024)
	// 	control->sl_index_count_log2 = 4;
	// else
	// 	control->sl_index_count_log2 = 5;

	control->sl_index_count_log2 = CALC_SL_INDEX_COUNT_LOG2(max_pool_size);
	...
	/* The above can all be calculated using macros. */

	size_t mem_size_min = sizeof(control_t) + (sizeof(*control->sl_bitmap) * control->fl_index_count) +
	...
	tlsf_assert(mem_size > mem_size_min && "pool size too small for tlsf control structure");
...
#if _DEBUG
	printf("real control size: %zu,macro calculated size: %zu\n", mem_size_min, TLSF_CALC_SZIE(max_pool_size));
	tlsf_assert(mem_size_min == TLSF_CALC_SZIE(max_pool_size) && "control structure true size does not match macro calculated size");
#endif
	...
	control->fl_bitmap = 0;
	control->sl_bitmap = tlsf_cast(size_t *, align_ptr(control + 1, sizeof(*control->sl_bitmap)));
	control->blocks = tlsf_cast(block_header_t **, align_ptr(control->sl_bitmap + control->fl_index_count, sizeof(*control->blocks)));
...
int tlsf_check(tlsf_t tlsf)
{
	...
	for (i = 0; i < control->fl_index_count; ++i)
	{
		for (j = 0; j < control->sl_index_count; ++j)
		{
			...
			{
				int fli, sli;
				const int is_block_free = block_is_free(block);
...
size_t tlsf_size(tlsf_t tlsf)
{
	...
	control_t *control = tlsf_cast(control_t *, tlsf);
	size_t control_size = sizeof(control_t) + (sizeof(*control->sl_bitmap) * control->fl_index_count) +
						  (sizeof(*control->blocks) * (control->fl_index_count * control->sl_index_count));
	return control_size;
}
...
#if _DEBUG
int test_macro_calculation()
{
	for (size_t i = 0; i < sizeof(size_t) * 8; i++)
	{
		if ((size_t)TLSF_LOG2_CEIL((size_t)1 << i) != (size_t)tlsf_log2_ceil((size_t)1 << i))
		{
			tlsf_printf("tlsf_log2_ceil: %u %u\n", (unsigned int)TLSF_LOG2_CEIL((size_t)1 << i), (unsigned int)tlsf_log2_ceil((size_t)1 << i));
		}
		tlsf_assert((size_t)TLSF_LOG2_CEIL((size_t)1 << i) == (size_t)tlsf_log2_ceil((size_t)1 << i));
	}
	return 0;
}
...
int test_ffs_fls()
{
	...
	test_macro_calculation();

	return rv;
}
#endif

tlsf_t tlsf_create(void *mem, size_t mem_size, size_t max_pool_size)
{
#if _DEBUG
	if (test_ffs_fls())
	{
	...
	control_t *control_ptr = control_construct(tlsf_cast(control_t *, mem), mem_size, max_pool_size);
	return tlsf_cast(tlsf_t, control_ptr);
}

tlsf_t tlsf_create_with_pool(void *mem, size_t mem_size, size_t max_pool_size)
{
	tlsf_t tlsf = tlsf_create(mem, mem_size, max_pool_size);
	if (tlsf != NULL)
	{
		tlsf_add_pool(tlsf, (char *)mem + tlsf_size(tlsf), mem_size - tlsf_size(tlsf));
	}
	return tlsf;
}
